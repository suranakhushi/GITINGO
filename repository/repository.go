...
`
	...
	if _, err := os.Stat(configPath); err == nil {
		localConfig, err := config.LoadConfig(configPath)
		if err != nil {
			return fmt.Errorf("failed to load local config: %w", err)
		}

		// If there are local overrides, use them
		if localConfig.User.Name != "" {
			localConfigContent = fmt.Sprintf("[user]\nname = %s\nemail = %s\n", localConfig.User.Name, localConfig.User.Email)
		}
	}

	// Write the final config to the local repository
